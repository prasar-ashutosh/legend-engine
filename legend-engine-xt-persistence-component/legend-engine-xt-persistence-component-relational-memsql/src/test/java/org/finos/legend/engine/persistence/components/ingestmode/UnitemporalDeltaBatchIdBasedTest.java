...
public class UnitemporalDeltaBatchIdBasedTest extends UnitmemporalDeltaBatchIdBasedTestCases
{
    ...
    @Override
    public void verifyUnitemporalDeltaWithOptimizationFilters(GeneratorResult operations)
    {
        List<String> preActionsSql = operations.preActionsSql();
        List<String> milestoningSql = operations.ingestSql();
        List<String> metadataIngestSql = operations.metadataIngestSql();
        ...
        String expectedMilestoneQuery = "UPDATE `mydb`.`main` as sink SET sink.`batch_id_out` = " +
                ...
                "WHERE batch_metadata.`table_name` = 'main')-1 WHERE (sink.`batch_id_out` = 999999999) AND " +
                "(sink.`id` >= 1) AND (sink.`id` <= 2) AND (EXISTS (SELECT * FROM `mydb`.`staging` as stage WHERE ((sink.`id` = stage.`id`) AND " +
        ...
        String expectedUpsertQuery = "INSERT INTO `mydb`.`main` " +
                ...
                "(sink.`digest` = stage.`digest`) AND ((sink.`id` = stage.`id`) " +
                "AND (sink.`name` = stage.`name`))))) AND (sink.`id` >= 1) AND (sink.`id` <= 2))";
    ...
    public void verifyUnitemporalDeltaWithOptimizationFiltersPlaceHolder(GeneratorResult operations)
    {
        ...
        String expectedMilestoneQuery = "UPDATE `mydb`.`main` as sink SET sink.`batch_id_out` = " +
                "(SELECT COALESCE(MAX(batch_metadata.`table_batch_id`),0)+1 FROM batch_metadata as batch_metadata " +
                "WHERE batch_metadata.`table_name` = 'main')-1 WHERE (sink.`batch_id_out` = 999999999) AND " +
                "(sink.`id` >= '{ID_LOWER_BOUND}') AND (sink.`id` <= '{ID_UPPER_BOUND}') AND (EXISTS (SELECT * FROM `mydb`.`staging` as stage WHERE ((sink.`id` = stage.`id`) AND " +
                "(sink.`name` = stage.`name`)) AND (sink.`digest` <> stage.`digest`)))";
        ...
        String expectedUpsertQuery = "INSERT INTO `mydb`.`main` " +
                ...
                "(NOT (EXISTS (SELECT * FROM `mydb`.`main` as sink WHERE (sink.`batch_id_out` = 999999999) AND " +
                "(sink.`digest` = stage.`digest`) AND ((sink.`id` = stage.`id`) " +
                "AND (sink.`name` = stage.`name`))))) AND (sink.`id` >= '{ID_LOWER_BOUND}') AND (sink.`id` <= '{ID_UPPER_BOUND}'))";
