...
public abstract class RelationalIngestorAbstract
{
    ...
    */
    ...
    private List<IngestorResult> ingest(List<DataSplitRange> dataSplitRanges)
    {
        ...
        {
            LOGGER.info(String.format(String.format("Starting Bulk Load")));
            return performBulkLoad(enrichedDatasets, transformer, planner, executor, generatorResult, enrichedIngestMode);
        }
        else
        {
            LOGGER.info(String.format(String.format("Starting Ingestion with IngestMode: {%s}", enrichedIngestMode.getClass().getSimpleName())));
                                                            ...
                                                            Optional<DataSplitRange> dataSplitRange)
    {
        ...
        {
            LOGGER.info(String.format(String.format("Obtained the next Batch id: %s", nextBatchId.get())));
